...
def make_atari(env_name: str,
    ...
    if 'FIRE' in env.unwrapped.get_action_meanings():  # noqa
...
class NoopResetEnv(gym.Wrapper):
    def __init__(self, env, noop_max=30):
        ...
        self.noop_action = 0
        assert env.unwrapped.get_action_meanings()[0] == 'NOOP'  # noqa
...
class EpisodicLifeEnv(gym.Wrapper):
    ...
    def reset(self):
        ...
        self.lives = self.env.unwrapped.ale.lives()  # noqa
        return obs
...
class FireResetEnv(gym.Wrapper):
    def __init__(self, env):
        ...
        assert env.unwrapped.get_action_meanings()[1] == 'FIRE'  # noqa
        assert len(env.unwrapped.get_action_meanings()) >= 3  # noqa
...
class ResizedAndGrayscaleEnv(gym.ObservationWrapper, ABC):
    ...
    def observation(self, observation):
        frame = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)  # noqa
        frame = cv2.resize(frame, (self.width, self.height), interpolation=cv2.INTER_AREA)  # noqa
        return frame
